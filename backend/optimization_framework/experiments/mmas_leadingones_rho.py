...
Standalone per-problem experiment: MMAS (``ant_colony_optimization``) on
LeadingOnes for several evaporation rates rho. Plots average fitness evaluations
to the optimum vs problem size n, one line per rho. Each generation constructs
``num_ants`` solutions, and all constructions count as fitness evaluations.

LeadingOnes is harder than OneMax (Theta(n^2 log n) for MMAS), so it uses
smaller sizes and a larger iteration cap than the OneMax experiment.

Output (under output/report_aco):
  - CSV : mmas_leadingones_rho_results.csv
  - PNG : mmas_leadingones_rho_scaling.png
  - JSON: mmas_leadingones_rho_summary.json

Usage:
    cd backend
    python3 -m optimization_framework.experiments.mmas_leadingones_rho
    python3 -m optimization_framework.experiments.mmas_leadingones_rho --sizes 20 40 --seeds 2 --max-iterations 50000
"""
...
from optimization_framework.experiments.report_aco_utils import (
...
NUM_ANTS = 10
PROBLEM_SIZES = [200, 300, 400, 500, 600]
DEFAULT_SEEDS = 10
...
def run_single_trial(n: int, rho: float, seed: int, max_iterations: int = MAX_ITERATIONS,
    ...
    best, iterations, _, _, fitness_evals, _c, _h = ant_colony_optimization(
        fitness_fn=fitnessLeadingOnes, bit_length=n, rho=rho,
...
def run_experiment(seeds: int = DEFAULT_SEEDS, sizes: List[int] = None,
    ...
    print(f"MMAS rho-sweep on LeadingOnes | rhos={rhos} | sizes={sizes} | seeds={seeds} | num_ants={num_ants}")
...
def save_results(all_results, aggregated):
    write_csv(all_results, OUTPUT_DIR / "mmas_leadingones_rho_results.csv",
              ["problem", "n", "rho", "num_ants", "seed", "fitness_evaluations", "iterations",
               "final_fitness", "reached_optimum", "elapsed_seconds"])
    write_json(aggregated, OUTPUT_DIR / "mmas_leadingones_rho_summary.json")
...
def plot_results(aggregated):
    ...
    ax.set_title("MMAS on LeadingOnes for Different Evaporation Rates", fontsize=14, fontweight="bold", pad=20)
    ax.grid(True, alpha=0.3, linestyle=":")
    ax.legend(loc="upper left")
    out = OUTPUT_DIR / "mmas_leadingones_rho_scaling.png"
...
def print_summary(aggregated):
    print("\n" + "=" * 70)
    print("SUMMARY: MMAS on LeadingOnes - evaporation rate sweep")
    print("=" * 70)
    for rho in sorted(aggregated.keys()):
        print(f"\n  ρ = {rho}")
...
def main():
    p = argparse.ArgumentParser(description="MMAS LeadingOnes evaporation-rate sweep")
