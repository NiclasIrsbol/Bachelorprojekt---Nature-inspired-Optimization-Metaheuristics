...
Output:
  - CSV: leadingones_mutation_scaling_results.csv
  - PNG: leadingones_mutation_scaling_comparison.png (convergence curves + theory)
  - JSON: leadingones_mutation_scaling_data.json (detailed results)
"""
...
def theoretical_runtime(n: float) -> float:
    """
    Theoretical expected runtime for (1+1) EA on LeadingOnes.
    Formula: 0.8591*n^2 (from literature)
    """
    if n <= 0:
        return 0
    return 0.8591*n**2
...
def run_single_trial(
    ...
    """
    Run a single (1+1) EA trial on LeadingOnes(n).
    ...
    final_fitness = fitnessLeadingOnes(best_bitstring)
...
def run_experiment(seeds: int = DEFAULT_SEEDS):
    ...
    print(f"Starting (1+1) EA LeadingOnes scaling experiment")
...
def save_results(all_results: List[Dict], aggregated: Dict):
    ...
    csv_file = OUTPUT_DIR / "leadingones_mutation_scaling_results.csv"
    json_file = OUTPUT_DIR / "leadingones_mutation_scaling_data.json"
...
def plot_results(aggregated: Dict):
    ...
    ax.set_title(
        "(1+1) EA on LeadingOnes: Mutation Rate Comparison vs. Theoretical Runtime",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    plot_file = OUTPUT_DIR / "leadingones_mutation_scaling_comparison.png"
...
def print_summary(aggregated: Dict):
    ...
    print("\n" + "="*80)
    print("SUMMARY: (1+1) EA on LeadingOnes - Mutation Rate Comparison")
    print("="*80)
...
def main():
    parser = argparse.ArgumentParser(
        description="(1+1) EA LeadingOnes scaling with mutation rate comparison"
    )
    parser.add_argument(
