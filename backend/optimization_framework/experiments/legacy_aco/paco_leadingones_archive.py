"""
Report section 8.5.1 (P-ACO, Bit strings -- LeadingOnes): archive-size sweep.

Standalone per-problem experiment: P-ACO (``population_based_aco``) on
LeadingOnes for several archive sizes K (num_ants and q0 fixed). Plots average
fitness evaluations to the optimum vs problem size n, one line per K.

LeadingOnes is harder than OneMax, so this uses smaller sizes and a larger
generation cap than the OneMax experiment.

Output (under output/report_aco):
  - CSV : paco_leadingones_archive_results.csv
  - PNG : paco_leadingones_archive_scaling.png
  - JSON: paco_leadingones_archive_summary.json

Usage:
    cd backend
    python3 -m optimization_framework.experiments.legacy_aco.paco_leadingones_archive
    python3 -m optimization_framework.experiments.legacy_aco.paco_leadingones_archive --sizes 20 40 --seeds 2 --max-iterations 50000
"""
...
NUM_ANTS = 10
Q0 = 0.9
PROBLEM_SIZES = [200, 400, 600, 800, 1000]
DEFAULT_SEEDS = 10
MAX_ITERATIONS = 200000  # generations; x num_ants evaluations per generation
...
def run_single_trial(n: int, archive_size: int, seed: int,
    ...
    best, iterations, _, _, fitness_evals, _c, _h = population_based_aco(
        fitness_fn=fitnessLeadingOnes, bit_length=n, archive_size=archive_size,
        max_iterations=max_iterations, num_ants=num_ants, q0=q0,
    )
    elapsed = time.time() - start
    final_fitness = fitnessLeadingOnes(best) if isinstance(best, str) else best["fitness"]
    return {
...
def run_experiment(seeds: int = DEFAULT_SEEDS, sizes: List[int] = None,
    ...
    print(f"P-ACO archive-sweep on LeadingOnes | K={archive_sizes} | sizes={sizes} | seeds={seeds} | num_ants={num_ants}")
...
def save_results(all_results, aggregated):
    write_csv(all_results, OUTPUT_DIR / "paco_leadingones_archive_results.csv",
              ["problem", "n", "archive_size", "num_ants", "q0", "seed",
               "fitness_evaluations", "iterations", "final_fitness", "reached_optimum", "elapsed_seconds"])
    write_json(aggregated, OUTPUT_DIR / "paco_leadingones_archive_summary.json")
...
def plot_results(aggregated):
    ...
    ax.set_title("P-ACO on LeadingOnes for Different Archive Sizes", fontsize=14, fontweight="bold", pad=20)
    ax.grid(True, alpha=0.3, linestyle=":")
    ax.legend(loc="upper left")
    out = OUTPUT_DIR / "paco_leadingones_archive_scaling.png"
...
def print_summary(aggregated):
    print("\n" + "=" * 70)
    print("SUMMARY: P-ACO on LeadingOnes - archive-size sweep")
    print("=" * 70)
    for k in sorted(aggregated.keys()):
        print(f"\n  K = {k}")
...
def main():
    p = argparse.ArgumentParser(description="P-ACO LeadingOnes archive-size sweep")
